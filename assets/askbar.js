...
(function () {
  ...
  var ENT = {
    ...
    dongxing: { nm: '句子懂行 · 记忆', ds: '把资料和业务规则变成 AI 员工的记忆', ic: 'fa-book-bookmark', href: P('dongxing.html') },
  ...
  var ROUTES = [
    ...
    { re: /(价格|报价|多少钱|收费|计价|预算|成本)/i, a: '方案和报价要看你的业务场景与部署方式，这里没法直接给数。留个联系方式，工作日当天会有顾问带着你所在行业的方案联系你。', cards: [], lead: true },
    ...
    { re: /(客服|售后|投诉|工单|接待)/i, a: 'AI 客服从售前咨询到售后投诉都能接，用的是 5 年积累的 BadCase 标注数据，能看出客户的意图和情绪，处理不了的转人工。', cards: ['service'] },
    { re: /(mio|办公|写方案|写报告|周报|ppt|海报|查资料|调研|搭子)/i, a: 'Mio 是给每个员工配的 AI 办公搭子：写方案、做 PPT、理数据、查资料，一句话布置，交回能直接验收的成果。企业统一开通，注册就能用。', cards: ['mio'] },
    { re: /(数据|报表|图表|问数|分析|看板|bi)/i, a: 'Mio 接着企业自己的数据：一句话问经营数据，秒级出图表，一层层追问到底，答案带出处；想盯的指标一句话搭成看板，异动主动提醒。不写 SQL、不约 BI。', cards: ['mio'] },
    { re: /(知识|文档|资料|检索|出处|知识库|记忆|本体|表结构|业务规则|erp)/i, a: '句子懂行是 AI 员工的记忆：文档、表格、视频传进来自动归档成段落和问答，说法不一致的、没覆盖到的，自己列成待办。业务系统里的表结构和规则也放进来，AI 建初稿，人只审拿不准的。秒懂、Mio 和 AI 员工都从这里取，答案带出处、带版本。', cards: ['dongxing'] },
  ...
  function greet() {
    ...
    var hi = c ? ('你好，关于「' + (c.title || c.entity) + '」想了解什么？或问我任何关于 AI 员工怎么落地的问题。')
                : '你好，我是句子的 AI 客服 👋 问我任何关于 AI 员工怎么落地的问题：区别、接入、数据安全、各产品/岗位都行。';
    addAI(hi);
    suggest();
  }
  function suggest() {
